...
export default function AdminVisitorForm({ leases }: { leases: any[] }) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-1 md:col-span-2">
            ...
            <select
              ...
              name="lease_id"
              required
              className="w-full bg-[#0c1324] border border-[#4a4455] rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors"
            >
              <option value="">Select a unit/tenant...</option>
              {leases.map((l) => (
                <option key={l.lease_id} value={l.lease_id}>
                  {l.unit.unit_number} - {l.tenant.user_name}
                </option>
              ))}
            </select>
          </div>
