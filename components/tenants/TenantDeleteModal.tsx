...
import { removeTenant } from "@/app/admin/tenants/actions";
...
export default function TenantDeleteModal({ tenant, isOpen, onClose }: TenantDeleteModalProps) {
  ...
  const leaseCount = tenant.tenant_leases?.length || 0;
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-outline-variant/40 flex items-center justify-between bg-rose-500/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-bold text-white">Delete Tenant Account</h2>
              <p className="text-[11px] text-rose-300/80">Permanent removal of resident profile</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="p-6 space-y-4 text-xs">
          ...
          <p className="text-on-surface-variant leading-relaxed">
            Are you sure you want to delete resident <strong className="text-white font-semibold">{tenant.user_name}</strong>?
          </p>
          ...
          {leaseCount > 0 ? (
            <div className="p-3.5 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-300 flex items-start gap-2.5">
              ...
              <div>
                <strong className="font-semibold block">Attached Leases Detected</strong>
                <span>This resident has {leaseCount} active or past lease agreements attached. Deleting the account is restricted by database foreign keys.</span>
              </div>
            </div>
          ) : (
            <p className="text-on-surface-variant/80 text-[11px]">
              This action cannot be undone. Resident credentials and portal access will be permanently revoked.
            </p>
          )}

          <form action={formAction} id="delete-tenant-form">
