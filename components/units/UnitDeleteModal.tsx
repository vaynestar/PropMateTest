...
export default function UnitDeleteModal({ unit, isOpen, onClose }: UnitDeleteModalProps) {
  ...
  const hasLeases = unit.leases && unit.leases.length > 0;
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-outline-variant/40 flex items-center justify-between bg-rose-500/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-bold text-white">Delete Unit {unit.unit_number}</h2>
              <p className="text-[11px] text-rose-300/80">Permanent removal of unit from inventory</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="p-6 space-y-4 text-xs">
          ...
          <p className="text-on-surface-variant leading-relaxed">
            Are you sure you want to delete unit <strong className="text-white font-semibold">{unit.unit_number}</strong>?
          </p>
          ...
          {hasLeases ? (
            <div className="p-3.5 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-300 flex items-start gap-2.5">
              ...
              <div>
                <strong className="font-semibold block">Active Lease Detected</strong>
                <span>This unit currently has an active tenant agreement. Deleting the unit will fail until the lease is terminated.</span>
              </div>
            </div>
          ) : (
            <p className="text-on-surface-variant/80 text-[11px]">
              This action cannot be undone. Maintenance logs and unit history will be purged.
            </p>
          )}

          <form action={formAction} id="delete-unit-form">
