...
export default function AdminFacilitiesManager({
  ...
  /** Facility whose maintenance visit is being logged. */
  const [loggingFacility, setLoggingFacility] = useState<any | null>(null);
  const [logState, logAction, isLogging] = useActionState(logMaintenanceAction, null);

  useEffect(() => {
    if (logState?.success) {
      setLoggingFacility(null);
      showToast(logState.message, "success");
    }
  }, [logState]);
            ...
            >
              ...
              <div className="mb-3 grid grid-cols-2 gap-2">
                ...
                <button
                  ...
                  disabled={isPending || isUpdatingThis || isDeletingThis}
                  className="pressable flex items-center justify-center gap-1.5 rounded-lg border border-primary/30 bg-primary/10 py-1.5 text-xs font-semibold text-primary transition-colors hover:bg-primary/20 disabled:opacity-50"
                >
      ...
      {loggingFacility &&
        (() => {
          ...
          return (
            <div
              className="animate-fade-in fixed inset-0 z-[200] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm"
              onClick={() => setLoggingFacility(null)}
            >
              <div
                role="dialog"
                aria-modal="true"
